...
class Sys_Opt_Theme_View(object):
    ...
    def gen_menu(self, is_active, is_refresh=False):
        ...
        if not is_refresh:
            self.container_view = rtk.ContainerMgr.create(name='theme_opt_view')
        ...
        if event.down:
            if not cglobals.stop_action:
                ...
                if event.key == 'D-Pad Up':
                    ...
                    rtk.cfg_theme = current_option
                    rtk.cfg_playlist = current_option
                    utils.refresh_theme()
