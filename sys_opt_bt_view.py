...
class Sys_Opt_BT_View(object):
    ...
    def gen_menu(self, is_active, is_refresh=False):
        ...
        if not is_refresh:
            self.container_view = rtk.ContainerMgr.create(name='bluetooth_opt_view')
    ...
    def set_bt_joy_names(self,names=None):
        if names:
            for i, name in enumerate(names[:8]):
                self.values['pair_bt_joy_'+str(i)] = name
        else:
            for i in range(0,8):
                self.values['pair_bt_joy_'+str(i)] = '-'
        ...
        if event.down:
            if not cglobals.stop_action:
                ...
                if event.key == 'D-Pad Up':
                    ...
                    if current_option == 'search_bt_joys':
                        rtk.popup_msg.display(text='search_info', l_icon='magnifier')
                        cglobals.is_in_task = True
                        cglobals.event_mgr.submit_event('scan_bt')
                    elif 'delete_bt_joys' in current_option:
                        rtk.popup_msg.display(text='unpair_bt_info')
                        cglobals.is_in_task = True
                        cglobals.event_mgr.submit_event('unpair_bt')
                    else:
                        if self.__get_current_value() != '-':
                            rtk.popup_msg.display(text='pair_bt_info')
                            cglobals.is_in_task = True
                            cglobals.event_mgr.submit_event('pair_bt')
