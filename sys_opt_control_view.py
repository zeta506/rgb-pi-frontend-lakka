...
class Sys_Opt_Control_View(object):
    ...
    def gen_menu(self, is_active, is_refresh=False):
        ...
        self.non_switches = ('bluetooth','lgun_cfg','joy_map_0','joy_map_1','joy_map_2','joy_map_3','joy_map_4')
        self.option_list = rtk.RtkTextList(
            name         = 'control_opt_list',
            text         = self.options,
            is_active    = is_active,
            font         = 'list',
            translate    = True,
        ...
        self.values = {
            ...
            'lgun_cfg'    :(),
            'user_remaps' :('off','on'),
            'button_style':cglobals.button_styles,
            'menu_toggle' :('only_menu_btn','start_select','start_2s'),
            'joy_map_0'   :self.__get_joy_name(joy_index=0),
            'joy_map_1'   :self.__get_joy_name(joy_index=1),
            'joy_map_2'   :self.__get_joy_name(joy_index=2),
            'joy_map_3'   :self.__get_joy_name(joy_index=3),
            'joy_map_4'   :self.__get_joy_name(joy_index=4)
        }
        self.value_list = rtk.RtkTextList(
            name         = 'control_value_list',
            text         = 'dummy',
            is_active    = is_active,
            font         = 'list',
            translate    = True,
        ...
        if not is_refresh:
            self.container_view = rtk.ContainerMgr.create(name='control_opt_view')
    ...
    def __get_joy_name(self, joy_index):
        ...
        if joystick:
            joy_name = utils.prettify_joy_name(joystick['name'])
            joy_name = utils.fit_text(text=joy_name, mode='short_truncated')
        else:
            joy_name = '-'
        return joy_name
        ...
        for option in self.options:
            try:
                ...
                if not isinstance(self.values[option], tuple) and not isinstance(self.values[option], list):
                    if option == 'joy_map_0': values.append(self.__get_joy_name(joy_index=0))
                    elif option == 'joy_map_1': values.append(self.__get_joy_name(joy_index=1)),
                    elif option == 'joy_map_2': values.append(self.__get_joy_name(joy_index=2)),
                    elif option == 'joy_map_3': values.append(self.__get_joy_name(joy_index=3)),
                    elif option == 'joy_map_4': values.append(self.__get_joy_name(joy_index=4))
                else:
    ...
    def __update_info(self):
        ...
        if current_option == 'menu_toggle' and current_value == 'start_select':
            self.item_info.set_text(text='start_select_inf')
    ...
    def apply_option(self):
        current_option = self.__get_current_option()
        if current_option == 'button_style':
            cglobals.input_mgr.set_joy_btn_style(joy_index=0, style=rtk.cfg_button_style)
            ...
            if not cglobals.stop_action:
                ...
                if event.key == 'D-Pad Up':
                    ...
                    if current_option not in self.non_switches:
                        joy_name = self.__get_joy_name(joy_index=0).lower()
                        if current_option == 'button_style' and joy_name == 'namco guncon 2':
                            rtk.notif_msg.display(text='gun_style_msg', l_icon='forbidden')
                    ...
                    if current_option == 'bluetooth':
                        utils.goto_view('sys_opt_bt_view')
                    elif current_option == 'lgun_cfg':
                        if utils.is_tate():
                            rtk.notif_msg.display(text='no_tate', l_icon='forbidden')
                        elif not cglobals.input_mgr.lgun:
                            rtk.notif_msg.display(text='no_gun', l_icon='forbidden')
                        else:
                            utils.goto_view('lgun_cfg_view')
                    elif 'joy_map_' in current_option:
                        joy_index = int(current_option[-1])
                        joy_name = self.__get_joy_name(joy_index=joy_index).lower()
                        if joy_name != '-':
                            if joy_name == 'namco guncon 2':
                                rtk.notif_msg.display(text='forbidden_lgun_map', l_icon='forbidden')
                            else:
                                cglobals.joy_cfg_view.config_joy(index=current_option[-1])
