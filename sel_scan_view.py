...
class Sel_Scan_View(object):
    ...
    def __set_positions(self):
        if utils.is_tate():
            ...
            self.list_y = rtk.selector_list_y_tate - 8
            self.selector_y = rtk.selector_y_tate - 8
        else:
            self.help_y = rtk.help_y
            self.list_y = rtk.selector_list_y - 8
            self.selector_y = rtk.selector_y - 8
    ...
    def gen_menu(self, is_active, is_refresh=False):
        ...
        if not is_refresh:
            self.container_view = rtk.ContainerMgr.create(name='sel_scan_view')
        ...
        if event.down:
            if not cglobals.stop_action:
                ...
                if event.key == 'D-Pad Up':
                    ...
                    rtk.popup_msg.display(text='scanning', l_icon='magnifier')
                    if 'usa' in current_option:
                        rtk.cfg_scrap_region = 'usa'
                    elif 'eur' in current_option:
                        rtk.cfg_scrap_region = 'eur'
                    elif 'jap' in current_option:
                        rtk.cfg_scrap_region = 'jap'
                    cglobals.is_in_task = True
                    cglobals.event_mgr.submit_event('save_config')
                    cglobals.event_mgr.submit_event('scan_scrap_games')
