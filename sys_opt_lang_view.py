...
class Sys_Opt_Lang_View(object):
    ...
    def gen_menu(self, is_active, is_refresh=False):
        ...
        if not is_refresh:
            self.container_view = rtk.ContainerMgr.create(name='lang_opt_view')
        ...
        if event.down:
            if not cglobals.stop_action:
                ...
                if event.key == 'D-Pad Up':
                    ...
                    if current_option == 'lang_english':
                        rtk.cfg_language = 'en'
                    elif current_option == 'lang_spanish':
                        rtk.cfg_language = 'es'
                    elif current_option == 'lang_french':
                        rtk.cfg_language = 'fr'
                    elif current_option == 'lang_german':
                        rtk.cfg_language = 'de'
                    elif current_option == 'lang_italian':
                        rtk.cfg_language = 'it'
                    elif current_option == 'lang_portuguese':
                        rtk.cfg_language = 'pt'
                    elif current_option == 'lang_swedish':
                        rtk.cfg_language = 'se'
                    rtk.popup_msg.display(text='loading')
                    cglobals.is_in_task = True
                    cglobals.event_mgr.submit_event('refresh_lang')
