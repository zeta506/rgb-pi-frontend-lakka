...
class Sel_Shutdown_View(object):
    ...
    def gen_menu(self, is_active, is_refresh=False):
        ...
        if not is_refresh:
            self.container_view = rtk.ContainerMgr.create(name='sel_shutdown_view')
        ...
        if event.down:
            if not cglobals.stop_action:
                ...
                if event.key == 'D-Pad Up':
                    ...
                    if self.__get_current_option() == 'reboot':
                        utils.shutdown(reboot=True)
