...
class Sys_Opt_Network_View(object):
    def __init__(self,is_active=False):
        # Menu initialization
        self.gen_menu(is_active)

    ## TODO - put only Wifi conect option > scan ssids and display connect/ssids (similar to BT) > vkeyb to enter pwd
    ...
    def gen_menu(self, is_active, is_refresh=False):
        ...
        if not is_refresh:
            self.container_view = rtk.ContainerMgr.create(name='network_opt_view')
    ...
    def set_ssid_name(self,name):
        self.values['wifi_ssid'] = utils.fit_text(text=name, mode='short_truncated')
        ...
        if event.down:
            if not cglobals.stop_action:
                ...
                if event.key == 'D-Pad Up':
                    ...
                    if current_option == 'config_wifi':
                        utils.goto_view('sys_opt_wifi_view')
